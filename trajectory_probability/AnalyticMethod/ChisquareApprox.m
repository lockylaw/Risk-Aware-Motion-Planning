...
hold on
scatter(fig.ax_main, rand_x, rand_y, '.', 'g');
...
dis_rad = dis_size - sqrt(chi2inv(0.9, 2)*max(eig(model.robot_pos_cov)));


% % plot robot shape
fig.fig_big_robot = plot_ellipse_2D(ax_main, model.robot_pos, [1, 1], 0, ...
                'FaceColor', 'r', 'FaceAlpha', 0, ...
                'EdgeColor', 'r', 'EdgeAlpha', 0.6);

fig.fig_small_robot = plot_ellipse_2D(ax_main, model.robot_pos, [dis_rad, dis_rad], 0, ...
                'FaceColor', 'b', 'FaceAlpha', 0, ...
                'EdgeColor', 'b', 'EdgeAlpha', 0.6);

h = legend([fig.fig_big_robot, fig.fig_small_robot], 'MC', 'chi-square approx.');
set(h, 'FontSize', 32)
