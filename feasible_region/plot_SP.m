...
if model_type == 1
    ...
    [feasible_region_SP, X_SP, Y_SP] = ...
        probability_risk_SP(model);
...
pcolor(X_SP, Y_SP, feasible_region_SP')
shading interp
hold on
...
if model_type == 1
    ...
    title('Senario Programming, Gaussian')
    axis([-3 3 -3 3]);
elseif model_type == 2
    [~, region_MC] = contour(X, Y, collision_probability_MC', [0.03, 0.03], 'LineColor', 'r', ...
    'LineWidth', 2.0);
    ...
    title('Senario Programming, GMM')
    axis([-3 3 -3 3]);
end
    


fig_ell_obs = plot_ellipse_2D(gca, model.robot_pos, ...
            model.robot_size, 0, ...
            'FaceColor', 'r', 'FaceAlpha', 0.4, ...
            'EdgeColor', 'r', 'EdgeAlpha', 0.6, ...
            'HandleVisibility', 'off');
        
fig_obs_pos = plot(model.robot_pos(1), model.robot_pos(2), ...
                    'Color', 'g', ...
                    'Marker', '*', ... 
