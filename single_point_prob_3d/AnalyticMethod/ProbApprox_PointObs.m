...
% robot size
model.robot_size = [0.1; 0.1];
model.robot_ell_size = sqrt(model.robot_size(1)^2 + model.robot_size(2)^2);     
...
scatter(fig.ax_main, pd_1(:, 1), pd_1(:, 2), '.');

if_Collision = zeros(N, 1);
for i = 1:N
    for j = 1:model.obs_num
        dis = sqrt((pd(i, 1) - pd_obs(i, 2*j - 1))^2 + (pd(i, 2) - pd_obs(i, 2*j))^2);
        if dis < model.robot_ell_size
            if_Collision(i) = 1;
        end
    end
end
...
sigma_all = model.obs_pos_cov + model.robot_pos_cov;

radius = model.robot_size(1);
ColliProb_Approx = pi*radius^2 * 1/sqrt(det(2*pi*sigma_all)) * ...
    exp(-1/2 * (model.robot_pos - model.obs_pos)' * sigma_all^(-1) * ...
    (model.robot_pos - model.obs_pos));
